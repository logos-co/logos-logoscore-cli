...
TEST_F(ErrorPathTest, NoLoadNegativePaths) {
    ...
    // Calling a method on an unknown module must fail -- by ANSWERING. These two
    // used to pass on `timeout`'s 124 alone, i.e. by hanging.
    ...
    // Calling a method on a known-but-unloaded module must fail, same way.
