...
│   │   ├── daemon_state.cpp/h        # Write/read/cleanup <configDir>/daemon/daemon.json;
│   │   │                             # carries listeners + hashed-token table
│   │   └── token_store.cpp/h         # Named-token table (lives inside daemon.json's
│   │                                 # `tokens` array + raw daemon/tokens/<name>.json)
│   │
│   ├── client/                       # Client path (all subcommands)
│   │   ├── client.cpp/h              # Read <configDir>/client/client.json, connect to
...
main.cpp
  → Daemon::start(modulesDirs, persistencePath, transportInfos)
    ...
    6. Mint the auto token, hash it into daemon.json["tokens"], emit raw
       value into client/auto.json, register hashes with TokenManager
    7. Write <configDir>/daemon/daemon.json       // listeners + tokens array
       Write <configDir>/client/client.json      // local-default dial spec
    8. Print startup message to stdout
    9. logos_core_exec()                          // Qt event loop (blocks)
   10. On SIGINT/SIGTERM or shutdown RPC:
       logos_core_cleanup()
       Remove daemon/daemon.json
       exit(0)
```
...
entry under that module's `transports` array in `daemon/daemon.json`, and the
...
table inside `<configDir>/daemon/daemon.json["tokens"]` (`{name, hash,
...
main.cpp
  → Client::connect()
    1. Read <configDir>/client/client.json (dial spec + instance_id + token_file)
...
`daemon/daemon.json`. They talk exclusively to the daemon's `core_service`
module via the SDK's RPC mechanism, using whatever dial spec
`client/client.json` provides. This means the client path depends only on
...
| `shutdown()` | Schedules `QCoreApplication::quit()` after a 200ms delay (to allow the RPC response to be sent), then the daemon performs its normal cleanup (unload modules, remove `daemon/daemon.json`, exit) |
...
| Method | Description |
|--------|-------------|
| `Daemon::start(modulesDirs) -> int` | Init liblogos, register core_service, write `daemon/daemon.json` + emit `client/client.json` and `client/auto.json`, run event loop |
...
**Purpose:** Write, read, and clean up the daemon state file (`<configDir>/daemon/daemon.json`). Pure parse — no liveness check. Daemon-owned; the client never reads it.

**API:**

| Method | Description |
|--------|-------------|
| `DaemonStateFile::write(instanceId, pid, modulesDirs, modules, tokens)` | Write the daemon state file. `modules` describes per-module listeners; `tokens` is the list of `{name, hash, issued_at, expires_at, local_only}` rows. |
| `DaemonStateFile::read() -> DaemonState` | Parse the file. `DaemonState::fileOk` is true iff the file exists and holds a non-empty `instance_id` — says nothing about liveness. |
| `DaemonStateFile::remove()` | Remove the daemon state file. |

**State file format** (`<configDir>/daemon/daemon.json`):
...
**Purpose:** In-memory map of issued client tokens, persisted into the daemon state file. Plaintext tokens are never stored on the daemon side — only SHA-256 hashes in `daemon.json["tokens"]`. The raw value of each named token lives only in `daemon/tokens/<name>.json` at the moment of issuance, for the operator to copy off.

**API:**

| Method | Description |
|--------|-------------|
| `TokenStore(configDir)` | Owns the `<configDir>/daemon/` subtree. Seeds itself from `daemon.json["tokens"]`. |
...
| `revokeToken(name) -> bool` | Remove the `name` entry from `daemon.json["tokens"]` and delete `daemon/tokens/<name>.json`. |
...
3. Daemon writes `instance_id` into both `daemon/daemon.json` and the auto-emitted `client/client.json`
4. Client reads `instance_id` from `client/client.json` → sets `LOGOS_INSTANCE_ID=a3f1c8d20b4e` in its own process → now `LogosInstance::id("core_service")` returns the matching URL
5. Client connects via `LogosAPIClient` → reaches the correct daemon

The client.json carries `instance_id` rather than a hardcoded registry URL so the client can reconstruct URLs using the same `LogosInstance::id()` function the SDK uses internally.

The token is generated on daemon startup, hashed into `daemon.json["tokens"]`, and emitted raw to `client/auto.json` for the local-default client to pick up. Client commands read it automatically via `client.json`'s `token_file` pointer. For remote/CI usage, the token can also be passed via `LOGOSCORE_TOKEN` env var.
...
|--------|---------------------------|
| `Client::connect() -> bool` | Read `<configDir>/client/client.json`, set `LOGOS_INSTANCE_ID` from `instance_id`, build `LogosTransportConfig` from the dial spec, load token from `token_file` (or `LOGOSCORE_TOKEN` env), create `LogosAPIClient` targeting `"core_service"`, authenticate |
...
| Method | Description |
|--------|-------------|
| `Config::getToken() -> QString` | Resolve token: env var → `<configDir>/client/<token_file>` (path read from `client.json`) |
| `Config::loadClientSpec() -> ClientSpec` | Load `<configDir>/client/client.json` (dial spec + `instance_id` + `token_file`) |
...
2. `<configDir>/client/<token_file>` (`token_file` defaults to `auto.json` when `client.json` doesn't override it)
...
3. Write `~/.logoscore/daemon/daemon.json` (listeners + hashed-token table) and emit `~/.logoscore/client/client.json` + `~/.logoscore/client/auto.json` for the local client
4. `logos_core_exec()` (Qt event loop — blocks)
5. On SIGINT/SIGTERM: `logos_core_cleanup()`, remove `daemon/daemon.json`, exit
...
**Behavior:**
1. Reads `<configDir>/client/client.json` to learn how to dial. If missing or unparseable, prints "not running" and exits with code 1 (no point trying to connect).
...
# Or via the client/ tree (point client.json's token_file at a JSON
...
| `test_daemon_state.cpp` | Round-trip `daemon/daemon.json` — instance_id, pid, modulesDirs, per-module `transports` entries (local/tcp/tcp_ssl, codec defaulting), and the `tokens` array (`name, hash, issued_at, expires_at, local_only`). `fileOk` is independent of the pid (it's a parse check, not liveness). |
| `test_token_store.cpp` | Token issuance (including `--expires` and `--local-only`), duplicate-name rejection (unless `--replace`), revocation, list, persistence round-trip. Confirms `daemon.json["tokens"]` stores hashes only; plaintext lives in `daemon/tokens/<name>.json`. |
| `test_config.cpp` | Token resolution order (env var → `client/<token_file>`); `client.json` parsing. |
